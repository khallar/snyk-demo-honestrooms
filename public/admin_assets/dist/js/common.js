...
/**
 * admin verifed or not in rooms
 */
$(document).on('change','.admin_rooms',function(){
...
$(document).on('click','.resubmit_listing',function(){
        ...
        $('#resubmit_listing').css('display','none');
         window.location = APP_URL+'/admin/rooms';
});
$(document).on('click','.resubmit',function(){
    ...
    if(msg != ''){
       ...
       // alert(APP_URL+'/'+ADMIN_URL+'/resubmit_listing/'+$room_id+'/'+$msg);
        //window.location = APP_URL+'/'+ADMIN_URL+'/resubmit_listing/'+$room_id+'/'+$msg;
        $.ajax({
      type: "POST",
