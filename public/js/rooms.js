...
if($('#url_checkin').val() != '' && $('#url_checkout').val() != '')
{
    ...
    $('#number_of_guests').val($('#url_guests').val());
     // $('#number_of_month').val($('#url_month').val());
    ...
    // $('#message_number_of_month').val($('#url_month').val());
    ...
    $http.post('price_calculation', { checkin :checkin,checkout : checkout, guest_count : guest,   room_id : room_id }).then(function(response) 
     {
       ...
       $('.js-book-it-status').removeClass('loading');
        ...
        if(response.data.base_rooms_price>0){
        $('.payment_book').removeClass('hide');

        $('.booking_checkin').text($('#list_checkin').val());
        $('.booking_checkout').text($('#list_checkout').val());
        $('.month_price').text(response.data.base_rooms_price);
        if(response.data.security_fee>0){
            $('.security_fees').removeClass('hide');
            $('#security_fees').text(response.data.security_fee);
        }else{
            $('.security_fees').addClass('hide');
        }

        }else{
         $('.payment_book').addClass('hide');
        }

        if(response.data.base_rooms_price>0){
            $('.total_night_price').show();
            $('#total_night_price').text(response.data.base_rooms_price);
        }
        else{
            $('.total_night_price').hide();
        }
        ...
        //Rental Prepayment Fee Update
        if(response.data.rental_price>0){
            $('.rental_price').show();
            $('#rental_price').text(response.data.rental_price);
            $('#rental_percentage').text(response.data.rental_percentage);
        }
        else{
            $('.rental_price').hide();
        }
        if(response.data.overall_total>0){
            $('.total').show();
            $('#total').text(response.data.overall_total);
        }else{
            $('.total').hide();
        }
        if(response.data.total>0){
            $('.pay_total').show();
            $('#pay_total').text(response.data.total);
        }else{
            $('.pay_total').hide();
        }
        $('#total_night_count').text(response.data.total_nights);
        
       $('#rooms_price_amount').text(response.data.base_rooms_price);
...
$http.post(APP_URL+'/rooms/rooms_calendar', { data:data }).then(function(response) {
  ...
  $('#message_checkin').datepicker({
      ...
      onSelect: function (date,obj) {
        ...
        var number_of_month = $('#message_number_of_month').val() * 30;
...
//Number Of month changes 
$(document).on('change','#message_number_of_month',function(){

    var number_of_month = $('#message_number_of_month').val();

    var month = number_of_month * 30;

    var room_id = $scope.room_id;

    if($('input[name="message_checkin"]').val() != '')
    {
     var checkout = $('#message_checkin').datepicker('getDate'); 
     checkout.setDate(checkout.getDate() + month);
     ...
     var checkout_date = checkout.getDate();
        ...
        var checkout_year = checkout.getFullYear();
        var checkout_formatted_date = checkout_date+'-'+checkout_month+'-'+checkout_year;
        $('input[name="message_checkout"]').val(checkout_formatted_date);
