...
<div class="panel row-space-4">
  ...
  <div id="accept_decline" class="panel-body">
    ...
    <table class="table table-bordered">
      <tbody>
        ...
        <tr>
          <td>
              {{ ucfirst(trans_choice('messages.service_host_rule.day',$result->nights_count)) }}
          </td>
          <td>
            {{ $result->nights_count }}
          </td>
        ...
        <tr>
          ...
          <tr>
            <td>
              {{ trans('messages.rooms.total') }} <i class="icon icon-question icon-question-sign tool-amenity2" title="{{ trans('messages.new_rooms.collect_amount_guest') }}" rel="tooltip" ></i>
            </td>
            <td>
 ...
 <div class="panel-body text-center-on-sm billing_table">
                <div class="row row-table">
                   <div class="payment_book reservation_book">
                    <p>{{trans('messages.new_rooms.payments_collect_to_guest')}}</p>

                                     <ul>
                                      <li class="payment_book_head">
                                       <label>{{trans('messages.new_rooms.when_guest_arrive')}} (<span class="booking_checkin"> {{ $result->checkin_dmy }}</span>)</label>
                                       <p>{{trans('messages.new_rooms.monthly_rent')}}: <span class="lang-chang-label">{{$result->currency->symbol}}</span><span  class="month_price" value="">{{$result->per_night}}</span></p>
                                       @if($result->security > 0)
                                         <p class="security_fees">{{trans('messages.new_rooms.security_deposit')}}: <span class="lang-chang-label">{{$result->currency->symbol}}</span><span  id="security_fees" value="">{{$result->security}}</span></p>
                                       @endif
                                      </li>
                                     <li class="payment_book_head">
                                     ...
                                     <li class="payment_book_head">
                                       <label>{{trans('messages.new_rooms.when_guest_leave')}} (<span class="booking_checkout">{{ $result->checkout_dmy }}</span>)</label>
