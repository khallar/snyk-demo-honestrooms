...
<div class="col-lg-4 col-sm-12" style="top:1px;">
  ...
  <form accept-charset="UTF-8" action="{{ url('payments/book/'.$room_id) }}" id="book_it_form" method="post">
    ...
    <div id="pricing" itemprop="offers" itemscope="" class="">
      <div id="price_amount" class="book-it-price-amount pull-left h3 text-special"><span class="lang-chang-label">{{ $result->rooms_price->currency->symbol }}</span> <span  id="rooms_price_amount" class="lang-chang-label" value="">{{ $result->rooms_price->month }}</span>
            ...
            <div class="col-md-9 col-sm-9 lang-chang-label">
              <div class="row row-condensed">
                <div class="col-sm-6 space-1-sm lang-chang-label">
                 ...
                 <input readonly="readonly" type="hidden" ng-model="available_date" ng-init="available_date = '{{$result->available_date == '0000-00-00' ? '' : (strtotime($result->available_date) > strtotime(date('Y-m-d')) ? $result->available_date_mdy :date('F d, Y')  ) }}';">
                ...
                <!--  <input type="hidden" id="url_month" value="{{ @$numberofmonth }}" > -->
                ...
                <!-- Number of month updated in -->

               <!--  <div class="col-sm-6 space-1-sm">
                  <label for="checkout">
                    {{ trans('messages.new_rooms.number_month') }}
                  </label>
                 <!--  <input readonly="readonly" class="checkout ui-datepicker-target" onfocus="this.blur()" autocomplete="off" id="list_checkout" placeholder="{{ strtolower(DISPLAY_DATE_FORMAT) }}" type="text"> --
                 number_of_month define StartserviceProvider
                 <select id="number_of_month" name="number_of_month">
          ...
          <div class="js-book-it-enabled clearfix">
            <div class="js-subtotal-container book-it__subtotal panel-padding-fit" style="display:none;">
              <table class="table table-bordered price_table" >
                <tbody>
                  <tr>
                    <td class="pos-rel room-night">
                     <!-- <span class="lang-chang-label"> {{ $result->rooms_price->currency->symbol }}</span>  <span  class="lang-chang-label" id="rooms_price_amount_1" value="">{{ $result->rooms_price->month }}</span> <span class="lang-chang-label">  x </span><span  id="total_night_count" value="">0</span>
                     <span ng-if="night_cnt <= 1">{{ trans_choice('messages.service_host_rule.month', 1) }}</span>
                     <span ng-if="night_cnt > 1">{{ trans_choice('messages.service_host_rule.month', 2) }}</span> -->
                     <span class="lang-chang-label">{{trans('messages.new_rooms.rent_per_month')}}</span>
                ...
                <tr class="rental_price">
                  <td class="pos-rel room-ser-fee">
                    {{ trans('messages.new_rooms.rental_prepayment') }}(<span id="rental_percentage"></span>%)

                   <!--  <i id="service-fee-tooltip"  rel="tooltip" class="icon icon-question" title="{{ trans('messages.rooms.24_7_help') }}"></i> -->

                  </td>
                  <td><span class="lang-chang-label">{{ $result->rooms_price->currency->symbol }}</span><span  id="rental_price" value="">0</span></td>
                </tr>
               ...
               <!--  <tr class="total">
                  ...
                  <tr class="pay_total">
                  <td>{{ trans('messages.new_rooms.total_to_pay_for_booking') }} <!-- <i id="service-fee-tooltip" rel="tooltip" class="icon icon-question" title="{{ trans('messages.new_rooms.remaining_amount_host') }}" ></i> --></td>
                  <td><span class="lang-chang-label">{{ $result->rooms_price->currency->symbol }}</span><span  id="pay_total" value="">0</span></td>
                </tr>
            ...
            <div class="payment_book hide">
              <p>{{trans('messages.new_rooms.payments_directly_to_host')}}</p>

              <ul>
                <li class="payment_book_head">
                  <label>{{trans('messages.new_rooms.when_you_arrive')}} (<span class="booking_checkin"></span>)</label>
                  <p>{{trans('messages.new_rooms.monthly_rent')}}: <span class="lang-chang-label">{{ $result->rooms_price->currency->symbol }}</span><span  class="month_price" value="">0</span></p>
                   <p class="security_fees hide">{{trans('messages.new_rooms.security_deposit')}}: <span class="lang-chang-label">{{ $result->rooms_price->currency->symbol }}</span><span  id="security_fees" value="">0</span></p>
                </li>
                 <li class="payment_book_head">
                 ...
                 <li class="payment_book_head">
                  <label>{{trans('messages.new_rooms.when_you_leave')}} (<span class="booking_checkout"></span>)</label>
      ...
      <div class="col-lg-8 lang-chang-label col-sm-12" id="details-column">
        ...
        <div class="row-space-8 row-space-top-8">
          ...
          <div class="space-4 space-top-4 show-sm">
            ...
            <div class="row">
              ...
              <div class="col-md-9 col-sm-12">
                <div class="row">
                  <div class="col-md-6 lang-chang-label col-sm-12">
                   <!-- {{-- @if($result->bed_type_name != NULL)
                    <div>{{ trans('messages.rooms.bed_type') }}: <strong>{{ $result->bed_type_name }}</strong></div>
                    @endif --}}-->
                    <div class="new_det_add ext_bor">{{ trans('messages.rooms.property_type') }}: <span>{{ $result->property_type_name }}</span></div>

                    <div class="new_det_add">{{ trans('messages.lys.accommodates') }}: <span>{{ $result->accommodates }}</span></div>
                  </div>
                  <div class="col-md-6">
            ...
            <hr>
            <!--
              * Service , host ,rules customization are update
              * SR_REQUEST_MODIF-13-02-2019
            -->
            <div class="row amenities">
              <div class="col-md-3 text-muted lang-chang-label col-sm-12">
               
                  {{ trans('messages.service_host_rule.bathrooms') }}
              ...
              <div class="col-md-9 col-sm-12">
                ...
                <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                   <div class="new_det_add">{{trans('messages.service_host_rule.private_bathrooms')}} : <span>{{$result->private_bathrooms}}</span></div>
                </div>
              </div>
             
              @if($result->private_bathrooms == 'No')
               <div class="row">
                <div class="col-md-12 lang-chang-label col-sm-12">
                 <div class="new_det_add">{{trans('messages.service_host_rule.shared_bathrooms')}} : <span>{{$result->bathrooms}} {{ trans('messages.service_host_rule.people_its_shared') }}</span></div>
               </div>
             </div>
             @endif
            ...
            @if($result->services_offer != '')
            <hr>
            <div class="row amenities">
              <div class="col-md-3 text-muted lang-chang-label col-sm-12">
               
                  {{ trans('messages.service_host_rule.services_offer') }}
               

              </div>
              <div class="col-md-9 col-sm-12 new_off">
                @if(in_array('1',$services_offer))
                <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                    <div class="new_det_add">
                    <p>{{ trans('messages.lys.weekly_cleaning_room') }}</p>
                  </div>
                </div>
              </div>
              @endif
               @if(in_array('2',$services_offer))
               <div class="row">
                <div class="col-md-12 lang-chang-label col-sm-12">
                 <div class="new_det_add"><p>{{ trans('messages.lys.bed_sheet_weekly_changed') }}</p></div>
               </div>
             </div>
             @endif
            </div>
          ...
          @endif
            ...
            <div class="row amenities">
              <div class="col-md-3 text-muted lang-chang-label col-sm-12">
                  {{ trans('messages.service_host_rule.host') }}
              </div>
              <div class="col-md-9 col-sm-12">
                <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                   <div class="new_det_add ext_bor"><h3 class="new_ex_head"> {{trans('messages.service_host_rule.lives_house_desc')}}:</h3><span class="detail_decs_view">{{ $result->lives_house_desc }}</span> </div>
                  </div>
                </div>
                ...
                <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                  <div class="new_det_add ext_bor"> <h3 class="new_ex_head"> {{trans('messages.service_host_rule.lives_house')}}:
                  </h3>
                      <span class="detail_decs_view">
                    @foreach($lives as $lives_val)
                    @if(in_array($lives_val->id,$lives_house))
                ...
                <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                   <div class="new_det_add ext_bor"><h3 class="new_ex_head">  {{trans('messages.service_host_rule.speak_languages')}}:
                     </h3>
                     <span class="detail_decs_view">
                    @foreach($language1 as $language_val)
                    @if(in_array($language_val->id,$speak_language))
                  
                   <p>{{ $language_val->name }}</p> 
               
                    @endif
                    @endforeach
                    
                    
                     @if(in_array('0',$speak_language))
                  
                      
                        <p>{{ trans('messages.service_host_rule.others') }}</p> 
                ...
                <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                   <div class="new_det_add ext_bor"><h3 class="new_ex_head">{{trans('messages.service_host_rule.pets_allowed')}}:
                     </h3>
                      <span class="detail_decs_view">
                    @foreach($pets as $petsval)
                    @if(in_array($petsval->id,$pets_val))
                  ...
                  <p>{{ $petsval->name }}</p>
                    ...
                    @endforeach
                     ...
                     @if(in_array('0',$pets_val))
                 
                    <p>{{ trans('messages.service_host_rule.others') }}</p> 
            
                    @endif
                    @if(in_array('no',$pets_val))
               
                    <p >{{ trans('messages.service_host_rule.nopets') }}</p> 
                ...
                <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                   <div class="new_det_add"><h3 class="new_ex_head"> {{trans('messages.service_host_rule.summary')}}:</h3><span class="detail_decs_view">{{ $result->host_summary }}</span> </div>
                  </div>
                </div>
          ...
          <div class="row">
            <div class="col-md-3 text-muted lang-chang-label col-sm-12">
              ...
              {{ trans('messages.lys.shared_spaces_public') }}
            ...
            <div class="col-md-9 col-sm-12">
              ...
              <div class="row">
                <div class="col-md-12 lang-chang-label col-sm-12">
                  <div class="new_det_add">
                    @if($shared_spaces["kitchen"])
                      <p>{{trans('messages.lys.shared_kitchen')}}</p>
                    @endif
                  </div>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col-md-12 lang-chang-label col-sm-12">
                  <div class="new_det_add">
                    @if($shared_spaces["living"])
                      <p>{{trans('messages.lys.shared_living_room')}}</p>
                    @endif
                  </div>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col-md-12 lang-chang-label col-sm-12">
                  <div class="new_det_add">
                    @if($shared_spaces["dining"])
                      <p>{{trans('messages.lys.shared_dining_room')}}</p>
                    @endif
                  </div>
                </div>
              </div>
              ...
              <div class="row">
                <div class="col-md-12 lang-chang-label col-sm-12">
                  <div class="new_det_add">
                    @if($shared_spaces["backyard"])
                      <p>{{trans('messages.lys.shared_backyard')}}</p>
                    @endif
                  </div>
                </div>
              </div>
            ...
            <div class="row amenities">
              <div class="col-md-3 text-muted lang-chang-label col-sm-12">
                  ...
                  {{ trans('messages.service_host_rule.rules') }}
              ...
              <div class="col-md-9 col-sm-12">
                ...
                <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                    <div class="new_det_add">
                  @if($result->allow_smoke == 'Yes') 
                  <p>{{trans('messages.service_host_rule.allow_smoke_at_home')}}</p>
                  @else
                  <p>{{trans('messages.service_host_rule.smoke_not_allow_at_home')}}</p>
                  @endif
                </div>
                </div>
              </div>
                 ...
                 <div class="row">
                  ...
                  @if($result->accept_pets == 'Yes') 
                  <p>{{trans('messages.service_host_rule.accept_pet_from_guest')}}</p>
                  @else
                  <p>{{trans('messages.service_host_rule.pets_not_accept_from_guest')}}</p>
                  @endif
                </div>
                </div>
              </div>
              ...
              <div class="row">
                  <div class="col-md-12 lang-chang-label col-sm-12">
                    <div class="new_det_add">
                  @if($result->invite_visitors == 'Yes') 
                  <p>{{trans('messages.service_host_rule.guests_invite_visitors')}}</p>
                  @else
                  <p>{{trans('messages.service_host_rule.guests_not_invite_visitors')}}</p>
                  @endif
                </div>
                </div>
              </div>
          ...
          <div class="row amenities">
            <div class="col-md-3 text-muted lang-chang-label col-sm-12">
              {{ trans('messages.lys.room_amenities_desc') }}
            </div>
              ...
              <div class="expandable-content-summary">
                <div class="row rooms_amenities_before" >
                  <div class="col-sm-6 lang-chang-label clrleft">
                    @php $i = 1 @endphp
                    @php $count = round(count($room_amenities) / 2) @endphp
                    @foreach($room_amenities as $all_amenities)
                      @if($i < 6)
                    ...
                    @php $i++ @endphp
                    @endforeach
                    @if(count($room_amenities) > 5)
                    <a class="expandable-trigger-more amenities_trigger" href="">
                      <span>+ {{ trans('messages.profile.more') }}</span>
                    </a>
                    @endif
                  </div>
                </div>
                <div class="row rooms_amenities_after " style="display:none;">
                  <div class="col-sm-6 lang-chang-label clrleft">
                    ...
                    @foreach($room_amenities as $all_amenities)
                      @if($all_amenities->status != null)
                      <div class="row-space-2 new_id {{ $all_amenities->type_id }}">
                ...
                <div class="expandable-content-summary">
                  <div class="row rooms_amenities_before" >
                    ...
                    <div class="col-sm-6 lang-chang-label clrleft">
                     ...
                     @php $count = round(count($other_amenities)/2) @endphp

                     @foreach($other_amenities as $all_amenities)
                     ...
                     <div class="row-space-2">
                      ...
                      <div class="row-space-1 text-muted">
                        ...
                        <i class="icon h3 icon-{{ $all_amenities->icon }}"></i>
                        &nbsp;
                        <span class="js-present-safety-feature future_basics"><span>
                          @if($all_amenities->status == null)
                          <del> 
                            @endif
          ...
          @if($result->service != '')
            ...
            <div class="row amenities">
              <div class="col-md-3 text-muted lang-chang-label col-sm-12">
                  ...
                  {{ trans('messages.service_host_rule.service') }}
              ...
              <div class="col-md-9 col-sm-12">
                ...
                @if(in_array($serviceval->id,$service_val))
               
                          <li>{{$serviceval->name}}</li>
            ...
            <div class="col-md-9 col-sm-12">
              <div class="row">
                <div class="col-md-6 lang-chang-label col-sm-6 fle">
                   <div class="new_det_add">
                  <div>{{ trans('messages.rooms.extra_people') }}: <span> 
                    @if($result->rooms_price->guests !=0 && $result->rooms_price->additional_guest!=0)

                   {{ $result->rooms_price->currency->symbol }} {{ $result->rooms_price->additional_guest }}   / {{ trans('messages.rooms.night_after_guest',['count'=>$result->rooms_price->guests]) }}
                ...
                <div class="col-md-6 ">
                 ...
                 <div class="new_det_add">{{ trans('messages.your_reservations.cancellation') }}:
                    <a href="{{ url('/home/cancellation_policies') }} " id="cancellation-policy"><span >{{trans('messages.your_reservations.view_policy')}}</span></a>
                  </div>
                </div>
                      ...
                      <form id="message_form" class="contact-host-panel" action="{{ url('/') }}/users/ask_question/{{ $result->id }}?src_url=rooms/{{ $result->id }}" method="POST">
                        ...
                        <div class="row-space-4 clearfix">
                          ...
                          <!-- <div class="col-4 input-col lang-chang-label rm_select">
                              ...
                              <div class="select select-block">
                               <select class="text-center" id="message_number_of_month" name="message_number_of_month" >
                                 @for($i=1;$i<= $number_of_month;$i++)
