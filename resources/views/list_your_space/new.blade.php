  ...
  <div class="panel lys-panel-header">
    <div class="panel-light become_new">
          <div class="becom_head">
            <h2 class="h5">{{ trans('messages.header.list_your_space_pre') }}</h2>
            <h1 class="h2">{{ trans('messages.header.list_your_space_title') }}</h1>
            <p class="text-lead">
              {{ trans('messages.header.list_your_space_post') }}
            </p>
          </div>
    </div>
...
{!! Form::open(['url' => 'rooms/create', 'class' => 'host-onboarding-form', 'accept-charset' => 'UTF-8' , 'name' => 'lys_new']) !!}
  ...
  <div class="fieldset fieldset_property_type_id">
    ...
    @for($i = 0; $i < 2; $i++)
      ...
      @endif
    @endfor
    <!-- Por el momento solo se permiten casa y departamento -->
...
<div class="active-selection" ng-show="property_type_id"  style="display:none;">
  <div data-type="property_type_id" class="selected-item property_type_id">
    ...
    <div class="active-panel" ng-click="property_type_rm()">
      ...
      <div ng-show="property_type_id == 1" class="active-message active-col">
        {{ trans('messages.lys.departament') }}
      </div>
      <div ng-show="property_type_id == 2" class="active-message active-col">
        {{ trans('messages.lys.house') }}
      </div>
    </div>
  ...
  <div class="space-top-3 space-1" ng-init="is_shared == 'No'">
    <div id="room-type-header" class="h5 text-light">
      ...
      <small>{{trans('messages.lys.room_type_pre')}}</small>
    </div>
  </div>
  <div class="fieldset fieldset_room_type">
    ...
    @for($i = 0; $i < 3; $i++)
      ...
      @endif
    @endfor
    <!-- Por el momento solo se aceptan las primeras 2 opciones -->
  ...
  <div data-type="room_type_id" class="selected-item room_type_id">
    ...
    <div class="active-panel" ng-click="room_type_rm()">
      ...
      <div class="active-message active-col">
        {{ trans('messages.lys.room_type_desc') }} 
        <span ng-show="is_shared == 'No'">
          {{ trans('messages.lys.room_type_desc_individual') }}
        </span>
        <span ng-show="is_shared == 'Yes'">
          {{ trans('messages.lys.room_type_desc_share') }}
        </span>
      </div>
    </div>
