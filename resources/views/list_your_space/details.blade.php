...
<div class="js-section">
  ...
  <div class="text-left space-top-3 space-1">
    <div id="shared-spaces-header" class="h5 text-light">
      <h4>{{ trans('messages.lys.shared_spaces') }}</h4>
    </div>
    <p class="text-muted">{!! trans('messages.lys.shared_spaces_desc') !!}</p>
    <div>
      <div class="js-section">
        <div style="display:none;" class="js-saving-progress saving-progress panel-shared-spaces">
          <h5>{{ trans('messages.lys.saving') }}...</h5>
        </div>
        <p>{{ trans('messages.lys.shared_spaces_subt') }}</p>
        <label>
          <input type="checkbox" name="shared_spaces_selected" class="shared-spaces-checkbox" ng-init="shared_spaces_kitchen={{ $shared_spaces["kitchen"] ? 'true' : 'false' }}" ng-model="shared_spaces_kitchen">
          {{ trans('messages.lys.shared_kitchen') }}
        </label>
        <label>
          <input type="checkbox" name="shared_spaces_selected" class="shared-spaces-checkbox" ng-init="shared_spaces_living={{ $shared_spaces["living"] ? 'true' : 'false' }}" ng-model="shared_spaces_living">
          {{ trans('messages.lys.shared_living_room') }}
        </label>
        <label>
          <input type="checkbox" name="shared_spaces_selected" class="shared-spaces-checkbox" ng-init="shared_spaces_dining={{ $shared_spaces["dining"] ? 'true' : 'false' }}" ng-model="shared_spaces_dining">
          {{ trans('messages.lys.shared_dining_room') }}
        </label>
        <label>
          <input type="checkbox" name="shared_spaces_selected" class="shared-spaces-checkbox" ng-init="shared_spaces_backyard={{ $shared_spaces["backyard"] ? 'true' : 'false' }}" ng-model="shared_spaces_backyard">
          {{ trans('messages.lys.shared_backyard') }}
        </label>
      </div>
    </div>
  </div>
 ...
 <div class="col-10 text-right next_step">
    <a data-prevent-default="" href="{{ url('manage-listing/'.$room_id.'/terms') }}" class="btn btn-large btn-primary next-section-button" ng-disabled="countSharedSpaceChecks() < 2">
