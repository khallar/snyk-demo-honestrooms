...
@endif
    ...
    <div class="row">
      <div class="col-md-5 col-lg-4 row-space-2 lang-ar-left payment_scroll" >
...
margin: 10px 0px !important;">@if($result->rooms_address->city !='') {{ $result->rooms_address->city }} , @endif 
                ...
                <table class="reso-info-table">
                  <tbody>
                    ...
                    <tr>
                      <td>
                        {{ ucfirst(trans_choice('messages.service_host_rule.day',$total_nights)) }}
                      </td>
                      <td>
                        {{ $total_nights }}
                      </td>
  ...
  <table id="billing-table1" class="reso-info-table billing-table">
    <tbody>
    ...
    <tr class="base-price">
      ...
      <!--  @if(Session::get('get_token')!='')
        ...
        {{ $result->rooms_price->currency->symbol }}
          @endif
          </span>{{ $price_list->base_rooms_price }}  x {{ $nights }} {{ trans_choice('messages.service_host_rule.month',$nights) }} -->
       <!--   <i id="service-fee-tooltip" rel="tooltip" title="{{ trans('messages.rooms.avg_night_rate') }}" style="position:relative;"  >
         
        </i>   -->  
        {{ trans('messages.new_rooms.rent_per_month') }}
      </span>
      </td>
      ...
      <span class="lang-chang-label"> @if(Session::get('get_token')!='')
        ...
        @else
        {{ $result->rooms_price->currency->symbol }}
          @endif</span><span >{{ $price_list->base_rooms_price }}</span></td>
...
<!-- <i id="service-fee-tooltip" class="icon icon-question" rel="tooltip" title="{{ trans('messages.rooms.24_7_help') }}" style="position:relative;"> -->
    ...
    @endif
     ...
     @if($price_list->rental_price)
    <tr class="service-fee">
      <td class="name pos-rel">
          {{ trans('messages.new_rooms.rental_prepayment') }}({{$price_list->rental_percentage}}%)
<!-- <i id="service-fee-tooltip" class="icon icon-question" rel="tooltip" title="{{ trans('messages.rooms.24_7_help') }}" style="position:relative;"> -->
      ...
      <span class="lang-chang-label"> @if(Session::get('get_token')!='')
        ...
        @else
        {{ $result->rooms_price->currency->symbol }}
          @endif</span><span >{{ $price_list->rental_price }}</span></td>
    </tr>
    @endif
    ...
    @endif
      ...
      <tr class="total">
        <td class="name">{{ trans('messages.new_rooms.total_to_pay_for_booking') }}<!-- <i id="service-fee-tooltip" rel="tooltip" class="icon icon-question" title="{{ trans('messages.new_rooms.remaining_amount_host') }}" ></i> --></td>
        <td class="val text-right">
        <span class="lang-chang-label">
          @if(Session::get('get_token')!='')
             {{ Session::get('currency_symbol') }}
          @else
           {{ $result->rooms_price->currency->symbol }}
          @endif
        </span><span id="payment_total">{{ $price_list->total }}</span></td>
      </tr>
  ...
  <table id="payment-total-table" class="reso-info-table billing-table">
    <tbody>
     <!--  <tr class="total">
        <td class="name"><span>{{ trans('messages.rooms.total') }}</span></td>
        <td class="text-special icon-dark-gray text-right"><span>
          ...
          @else
           {{ $result->rooms_price->currency->symbol }}
          @endif
        </span> <span id="payment_total">{{ $price_list->overall_total }}</span></td>
      </tr> -->
     <!--  <tr class="total">
        <td class="name"><span class="h4">{{ trans('messages.new_rooms.total_to_pay_for_booking') }}</span> </td>
  ...
  <div class="payment_book ">
              ...
              <ul>
                <li class="payment_book_head">
                  <label>{{trans('messages.new_rooms.when_you_arrive')}} (<span class="booking_checkin">{{ date($php_format_date, strtotime($checkin)) }} </span>)</label>
                  <p>{{trans('messages.new_rooms.monthly_rent')}}: <span class="lang-chang-label"> @if(Session::get('get_token')!='')
                 {{ Session::get('currency_symbol') }}
                  @else
                  {{ $result->rooms_price->currency->symbol }}
                 @endif</span><span  class="month_price" value="">{{ $price_list->base_rooms_price }}</span></p>
                   @if($price_list->security_fee > 0)
                   <p class="security_fees">{{trans('messages.new_rooms.security_deposit')}}: <span class="lang-chang-label">@if(Session::get('get_token')!='')
                 {{ Session::get('currency_symbol') }}
                  @else
                  {{ $result->rooms_price->currency->symbol }}
                 @endif</span><span  id="security_fees" value="">{{ $price_list->security_fee }}</span></p>
                 @endif
                </li>
                 <li class="payment_book_head">
                 ...
                 <li class="payment_book_head">
                  <label>{{trans('messages.new_rooms.when_you_leave')}} (<span class="booking_checkout">{{ date($php_format_date, strtotime($checkout)) }}</span>)</label>
...
<div class="panel-total-charge">
  ...
  <small><div>
    ...
    <span id="fx-messaging">{{ trans('messages.payments.exchange_rate_booking',['symbol'=>PAYPAL_CURRENCY_SYMBOL]) }} {{ $result->rooms_price->currency->original_symbol }}{{ $paypal_price_rate }} {{ $result->rooms_price->currency_code }} ({{ trans('messages.payments.host_listing_currency') }}).</span>
      ...
      <div id="content-container" class="col-md-7 lang-ar-right payment_conte">
