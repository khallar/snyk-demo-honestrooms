...
<main id="site-content" role="main" ng-controller="search-page" ng-init="setParams()">
  <div class="home_filter" ng-init="opened_filter = '';">
    <ul ng-cloak>
     ...
     <!-- {{-- <li class="frt">
        <button class="guestbut newt" id="gue1" data-target-div="month1" data-target-filter="month" ng-class="filter_active('month')" ng-click="update_opened_filter('month')">
          <span ng-if="!is_filter_active('month')">{{ trans_choice('messages.new_rooms.month',2) }}</span>
          <span ng-if="is_filter_active('month')">@{{search_month}} {{ trans_choice('messages.new_rooms.month',$month) }}</span>
        </button>
        <div class="guestbut1 month_opt" id="month1">
        
          <div class="nameguest nameguest1">
           
              <div class="select month_select" ng-init="search_month={{$month}}">
                <select name="month" class="month-select " ng-model="search_month" ng-value="{{$month}}">
            ...
            <div class="newed">
              <span class="newcan"><a href="javascript:void(0)" class="close_target" ng-click="reset_filters('month')">{{ trans('messages.your_reservations.cancel') }}</a></span>
              <span class="newapp"><a href="javascript:void(0)" ng-click="apply_filters('month')">{{ trans('messages.payments.apply') }}</a></span>
            </div>
          </div>
        </div>
      </li> --}}-->
          ...
          <div class="nameguest nameguest1">
            <div class="forgu">
              <div class="ad">
                ...
                <div class="valunum" ng-init="search_guest={{$guest}};accomodates ={{$number_of_accomodates}}">
   ...
   <!--    <li class="frt">
      ...
      <!--{{-- <li class="frt1">
        <button class="gut6 gtnew" data-target-filter="month" ng-class="filter_active('month')">
          <span ng-if="!is_filter_active('month')">{{ trans_choice('messages.new_rooms.month',2) }}</span>
          <span ng-if="is_filter_active('month')">@{{search_month}} {{ trans_choice('messages.new_rooms.month',2) }}</span>
        </button>
      </li> --}} -->
      <li class="frt1">
...
<div class="checkbox-group hosting_services_offer filters-section panel-body panel-light toggle-group" data-name="hosting_services_offer">
  <div class="row">
    <div class="col-lg-2 col-md-12 filter-label text-center-md">
      <label>{{ trans('messages.lys.services_offer') }}</label>
    </div>
    ...
    <div class="col-lg-9 col-md-11 right-pull">
      <div class="row row-condensed filters-columns">
        ...
        <div class="col-md-4">
          <label class="media checkbox text-truncate" title="{{ trans('messages.lys.weekly_cleaning_room') }}">
            <input type="checkbox" name="more_filter[]" value="1" class="pull-left services_offer" id="map-search-services_offer-1" ng-checked="{{ (in_array(1, $services_offer_selected)) ? 'true' : 'false' }}">
            {{ trans('messages.lys.weekly_cleaning_room') }}
          </label>
        </div>
        <div class="col-md-4">
          <label class="media checkbox text-truncate" title="{{ trans('messages.lys.bed_sheet_weekly_changed') }}">
            <input type="checkbox" name="more_filter[]" value="2" class="pull-left services_offer" id="map-search-services_offer-2" ng-checked="{{ (in_array(2, $services_offer_selected)) ? 'true' : 'false' }}">
            {{ trans('messages.lys.bed_sheet_weekly_changed') }}
          </label>
        </div>
    ...
    <div class="col-md-8 col-lg-7 col-md-offset-4 col-lg-offset-5 applycancel-search">
      <div class="row row-condensed space-sm-7"> 
        ...
        <div class="col-sm-4 apply-red">
          <button class="btn btn-block btn-primary apply-filters-btn" id="more_filter_submit" disabled="disabled" {{ (count($amenities_selected) || count($services_offer_selected) || count($property_type_selected)) ? '' : 'disabled' }} ng-click="apply_filter();">
...
<div class="search-results toggle-hide back-change" ng-init="bed_text='{{ trans_choice("messages.lys.bed",1) }}'; beds_text='{{ trans_choice("messages.lys.bed",2)}}'; per_night='{{ trans("messages.service_host_rule.per_month") }}'; review_text='{{ trans_choice("messages.header.review",1) }}'; reviews_text='{{ trans_choice("messages.header.review",2) }}';">
            ...
            <div class="panel-image listing-img">
              <div class="listing-description wl-data-@{{ rooms.id }}">
                <div class="summary">
                  <p>
                   @{{ rooms.summary }}
                   <a href="{{ url('s') }}/@{{ location }}?checkin=@{{ checkin }}&checkout=@{{checkut}}&guests=@{{ guests }}" id="tooltip-sticky-@{{ rooms.id }}" class="learn-more">{{ trans('messages.search.learn_more') }}</a>
                 </p>
...
<!--               <a href="{{ url('users/show/') }}/@{{ rooms.user_id }}" class="media-photo-badge pull-right card-profile-picture card-profile-picture-offset">
             ...
             <!--  <span class="pull-left" >@{{ rooms.beds }}
              ...
              <a href="{{ url('rooms') }}/@{{ rooms.id }}?checkin=@{{ checkin }}&checkout=@{{cheeckout}}&guests=@{{ guests }}" target="listing_@{{ rooms.id }}" class="text-normal">
...
<div class="nw_more">
  ...
  <!--   <h4>{{trans('messages.service_host_rule.rooms_private_house')}}</h4> -->
    <div class="avairoom" ng-init="search_private_bath = {{$private_bathrooms != '' ? $private_bathrooms : ''}};private_bathroom ='{{$private_bathrooms}}';">
      <div class="bedroom">{{trans('messages.service_host_rule.rooms_private_house')}}</div> 
        <div class="valunum1">
         
          <input type="radio" value="Yes" class="private_bathroom" name="private_bathroom"  id="private_bathrooms_Yes" data-saving="Yes" {{ $private_bathrooms == 'Yes' ? 'checked' : '' }}>
          <span>{{ trans('messages.service_host_rule.yes') }}</span>

           <input type="radio" value="No" class="private_bathroom" name="private_bathroom" id="private_bathrooms_No"  data-saving="No" {{ $private_bathrooms == 'No' ? 'checked' : '' }}>
          <span>{{ trans('messages.service_host_rule.no') }}</span>
        </div>
    </div>

    <div class="avairoom" ng-show="private_bathroom == 'No'" ng-init="search_bath = {{$bathrooms > 0 ? $bathrooms : 0}}">
      <div class="bedroom">{{trans('messages.service_host_rule.rooms_shared_house')}}</div>
        <div class="valunum1">
<ul class="list-unstyled amenity-list">
          @for($i=1;$i<=4;$i++)
          <li style="padding-bottom: 6px;" >
            <input type="radio" value="{{ $i }}" class="bathroom" id="bathrooms_{{$i}}" name="bathrooms" data-saving="{{ $i }}" {{ $shared_bathrooms == $i ? 'checked' : '' }}>
           <span> {{ ($i == '4') ? $i.'+' : $i }}</span>
         </li>
          @endfor
...
@endif
  ...
  <div class="amti">
    <h4>{{ trans('messages.lys.services_offer') }}</h4>
    <div class="amtmore">
      ...
      <div class="col-md-6">
        <div class="row">
          <div class="holecheck">
              <div class="">
                <input type="checkbox" id="services_offer_1"  value="1" class="services_offer" {{(in_array(1, $services_offer_selected)) ? 'checked' : ''}} />
                <label class="search_check_label" for="services_offer_1">{{ trans('messages.lys.weekly_cleaning_room') }}</label>
              </div>
          </div>
        </div>
      </div>
       ...
       <div class="col-md-6">
        <div class="row">
          <div class="holecheck">
              <div class="">
                <input type="checkbox" id="services_offer_2"  value="2" class="services_offer" {{(in_array(2, $services_offer_selected)) ? 'checked' : ''}} />
                <label class="search_check_label" for="services_offer_2">{{ trans('messages.lys.bed_sheet_weekly_changed') }}</label>
              </div>
          </div>
        </div>
      </div>
...
<!--       <div class="seead seead1">
      <div class="seeallame">{{ trans('messages.header.seeall') }} {{ trans('messages.lys.services_offer') }}</div>
      <div class="cls">{{ trans('messages.home.close') }} {{ trans('messages.lys.services_offer') }}</div>
      <span><i class="fa fa-angle-down" aria-hidden="true"></i></span></div> -->
    </div>
...
<ul class="list-unstyled amenity-list">
          @for($i=1;$i<=4;$i++)
          <li style="padding-bottom: 6px;">
            <input type="radio" value="{{ $i }}" class="bathroom" name="bathrooms" data-saving="{{ $i }}" {{ $bathrooms == $i ? 'checked' : '' }}>
           <span> {{ ($i == '4') ? $i.'+' : $i }}</span>
         </li>
          @endfor
 ...
 -->
  ...
  <div class="name34 room-type-filter">
    ...
    <div class="foredhole">
      <div class="checking">
      <input type="radio" value="Yes" id="mob_private_bathrooms_Yes" name="mob_private_bathroom" class="private_bathroom"  {{ $private_bathrooms == 'Yes' ? 'checked' : '' }}/><label for="mob_private_bathrooms_Yes" ></label>
      </div>
      <label for="mob_private_bathrooms_Yes" class="for rt_label">
        <div class="room_type1 ent">{{ trans('messages.service_host_rule.yes') }}</div>
    ...
    <div class="foredhole">
      <div class="checking">
      <input type="radio" value="No" id="mob_private_bathrooms_No" name="mob_private_bathroom"  class="private_bathroom"  {{ $private_bathrooms == 'No' ? 'checked' : '' }}/><label for="mob_private_bathrooms_No" ></label>
      </div>
      <label for="mob_private_bathrooms_No" class="for rt_label">
        <div class="room_type1 ent">{{ trans('messages.service_host_rule.no') }}</div>
  ...
  <div class="name34 room-type-filter" ng-show="private_bathroom == 'No'">
    ...
    <div class="foredhole">
      <div class="checking">
      <input type="radio" value="{{$i}}" id="mob_bathrooms_{{$i}}" name="mob_bathrooms" class="bathroom"  {{ $shared_bathrooms == $i ? 'checked' : '' }}/><label for="mob_bathrooms_{{$i}}" ></label>
      </div>
      <label for="mob_bathrooms_{{$i}}" class="for rt_label">
        <div class="room_type1 ent"> {{ ($i == '4') ? $i.'+' : $i }} </div>
  ...
  <div class="slidedow slidedow1">
    ...
    <div class="col-md-6">
      <div class="row">
        <div class="holecheck">
            <div class="">
              <input type="checkbox" id="mob_services_offer_1" value="1" class="services_offer" {{in_array(1, $services_offer_selected) ? 'checked' : ''}} />
              <label class="search_check_label" for="mob_services_offer_1">{{ trans('messages.lys.weekly_cleaning_room') }}</label>
            </div>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <div class="row">
        <div class="holecheck">
            <div class="">
              <input type="checkbox" id="mob_services_offer_2" value="2" class="services_offer" {{in_array(2, $services_offer_selected) ? 'checked' : ''}} />
              <label class="search_check_label" for="mob_services_offe_2">{{ trans('messages.lys.bed_sheet_weekly_changed') }}</label>
            </div>
        </div>
      </div>
    </div>
...
{{--<div class="morefit6">
  <div class="cls12">
    <div class="allfill" ng-click="reset_filters('month')">
      <i class="fa fa-times clsfa6" aria-hidden="true">
        
      </i>
    </div>
    <div class="allf text-center">
      {{ trans_choice('messages.new_rooms.month',2) }}
      <span>
        <a class="text-right clsfa6" href="javascript:void(0)" ng-click="reset_filters('month')">
  ...
  <div class="adl">
    <div class="forgu">
      <div class="ad">{{ trans_choice('messages.new_rooms.month',1) }}</div>
      <div class="valunum select">
        <select name="month" class="month-select " ng-model="search_month" ng-value="{{$month}}">
          @for($i=1;$i<=$number_of_month;$i++)
            <option value="{{ $i }}"> {{$i}} </option>
          @endfor
  ...
  <div class="morebot">
    <div class="holebot">
      <a href="javascript:void(0)" ng-click="apply_filters('month')" class="seehome">{{ trans('messages.wishlist.see_homes') }}</a>
    </div>
  </div>
</div> --}}
<div class="morefit2">
