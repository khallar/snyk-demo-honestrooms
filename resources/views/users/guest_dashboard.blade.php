...
@endif
    ...
    <div class="col-md-9 dash-right-sec pull-left">
    ...
    @endif
      ...
      <div class="panel space-4">
        ...
        <ul class="list-layout">
            ...
            @if($all->reservation_id != 0)
            <li id="thread_{{ $all->id }}" class="panel-body thread-read thread">
  <div class="row">
    <div class="col-sm-2 col-md-5 col-lg-3 lang-chang-label thread-author">
      <div class="row row-table">
       <div class="thread-avatar col-md-5 lang-chang-label">        
          <a data-popup="true" href="{{ url('/') }}/users/show/{{ @$all->user_details->id }}">
            <img height="50" width="50" title="{{ $all->message_type == '13' ? 'Admin' : @$all->user_details->first_name }}" src="{{  $all->message_type == '13' ? $all->admin_profile_src:$all->user_details->profile_picture->src }}" class="media-round media-photo" alt="{{ $all->message_type == '13' ? 'Admin':$all->user_details->first_name }}"></a>
        </div>
         <div class=" thread-name list_name1">
           <span class="text_name3"> {{ $all->message_type == '13' ? trans('messages.new_rooms.admin') : $all->user_details->first_name }}</span>
    ...
    @if(@$all->host_check ==1 && @$all->reservation->status == 'Pending' && $all->message_type != 13)
    <a class="link-reset text-muted text_prag" href="{{ url('reservation')}}/{{ @$all->reservation_id }}">
      <span class="thread-subject ng-binding unread_message">{{ @$all->message }}</span>
        ...
        <!--  <span> ({{  (date($php_format_date, strtotime( @$all->reservation->checkin))) }}, 
                {{  (date($php_format_date, strtotime( @$all->reservation->checkout))) }})</span> -->
                @endif
        
        </span>       
     
</a>
    @elseif(@$all->host_check ==1 && @$all->reservation->status != 'Pending'  && $all->message_type != 13)
    ...
    @endif
    @if(@$all->guest_check !=0 && $all->message_type != 13) 
    ...
    @endif
    @if($all->message_type == 13) 
    <a class="link-reset text-muted text_prag" href="{{ url('messaging/admin')}}/{{ @$all->reservation_id }}">
      <span class="thread-subject ng-binding unread_message">{{ @$all->message }}</span>
...
@else
 <div class="next_list">
  <div class="status_list">
   <span class="label label-info">{{trans('messages.lys.resubmit')}}</span>
  </div></div>

@endif
@endif
</div>
   <div class="col-md-2 col-sm-7 col-lg-3 lang-chang-label mob_off">
    @if(@$all->reservation->list_type != 'Experiences' || @$all->reservation->type != 'contact' )
     @if($all->message_type != 13) 
      <!--  <span class="label label-{{ @$all->reservation->status_color }}">{{ @$all->reservation->status_language }}</span>
        <br>
