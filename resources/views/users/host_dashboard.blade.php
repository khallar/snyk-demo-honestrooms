...
<div class=" thread-name list_name">
    ...
    <a class="link-reset text-muted1" href="{{ url('reservation')}}/{{ @$all->reservation_id }}">
        ...
        <span class="text-muted1 show-lg show-sm dash_board_dec">
 ...
 <li id="thread_{{ $all->id }}" class="panel-body thread-read thread">
  <div class="row">
    ...
    <div class="col-sm-2 col-md-4 col-lg-3 lang-chang-label thread-author inbox_history1">
      <div class="row row-table">
        <div class="thread-avatar col-md-5 lang-chang-label">
         <img height="50" width="50" title="Admin" src="{{ url() }}/admin_assets/dist/img/avatar04.png" class="media-round media-photo" alt="Admin"></a>
        </div>
        <div class=" thread-name list_name1">
          <span class="text_name3">  Admin</span>
          <br>
           <span class="thread-date"> {{ $all->created_time }}</span>
        </div>
      </div>
...
<div class=" thread-name list_name">
    ...
    <div class="common_inbox">
     <a class="link-reset text-muted1" href="{{ url('messaging/admin')}}/{{ @$all->reservation_id }}">
         <span class="thread-subject ng-binding unread_message">{{ @$all->message_split }}</span>
         </a>
...
<div class=" thread-name list_name">
    ...
    <div class="common_inbox">
     <a class="link-reset text-muted1" href="{{ url('messaging/admin')}}/{{ @$all->reservation_id }}">
         <span class="thread-subject ng-binding unread_message resubmit_msg1" data-msg ="{{$all->message}}" data-msg_id="{{$all->id}}">{{ @$all->message_split }}</span>
        <span class="text-muted1 show-lg show-sm dash_board_dec">
        {{$all->rooms->name}}(
             @if($all->rooms_address->address_line_1 !='' || $all->rooms_address->address_line_2 != '')
              <span class="street-address" >{{ $all->rooms_address->address_line_1 }} {{ $all->rooms_address->address_line_2 }},</span> @endif   
              @if($all->rooms_address->city !='' )         
              <span class="locality" >{{ $all->rooms_address->city }}, </span> 
              @endif
              @if($all->rooms_address->state !='' ) 
...
<div class="next_list">
  <div class="status_list">
   <span class="label label-info">{{trans('messages.new_rooms.resubmit')}}</span>
