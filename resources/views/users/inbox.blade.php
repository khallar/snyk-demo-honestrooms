...
<main id="site-content" role="main" ng-controller="inbox">
  ...
  <div class="page-container-responsive page-container-inbox space-4 space-top-4">
      ...
      <div class="panel">
        ...
        <ul id="threads" class="list-layout panel-body">
          <li id="thread_153062093" class="panel-body js-thread is-starred thread"  ng-repeat="all in message_result.data" ng-class="(all.read == '1') ? ' thread--read' : ''" ng-cloak>
            <div class="row"  ng-if="all.user_from != all.user_to"> 
    ...
    <div class="col-sm-2 col-md-4 col-lg-3 lang-chang-label thread-author inbox_history1">
      <div class="row row-table">
        <div class="thread-avatar col-md-5 lang-chang-label">
          <img height="50" width="50" title="Admin" ng-src=" @{{all.admin_profile_src }}" class="media-round media-photo" alt="Admin">
        </div>
        <div class=" thread-name list_name1">
         <span class="text_name3"> {{trans('messages.new_rooms.admin')}}</span>
...
<div class="col-sm-7 col-md-5 col-lg-6 thread-body lang-chang-label inbox_history2" >
  ...
  <div class="common_inbox">
    <a class="link-reset text-muted1" href="{{ url('messaging/admin')}}/@{{ all.reservation_id }}">
      
       <span class="thread-subject text_prag resubmit_msg1" ng-class="(all.read == '1') ? '' : 'unread_message'" data-msg ="@{{all.message}}" data-msg_id="@{{all.id}}">
           @{{ all.message_split }}
        </span>

        <div class="text-muted show-lg show-sm dash_board_dec">

            <span ng-if="all.list_type == 'Rooms'">
            @{{all.rooms.name}} (
              <span class="street-address"  ng-if="all.rooms_address.address_line_1 || all.rooms_address.address_line_2 ">@{{ all.rooms_address.address_line_1 }} @{{ all.rooms_address.address_line_2 }},</span>             
              <span class="locality" ng-if= "all.rooms_address.city">@{{ all.rooms_address.city }}, </span> 
              <span class="region">@{{ all.rooms_address.state }}</span>
              )
            </span>
...
<div class="next_list">
  <div class="status_list">
    <span class="label label label-info"> {{trans('messages.new_rooms.resubmit')}}</span>
          </div>
        </div>
...
<div class="col-md-3 col-sm-7 col-lg-3 thread-label lang-chang-label inbox_history">
      <div class="row">
        <div  class="col-sm-12 col-md-12 col-lg-6 acceptrej lang-chang-label" >
           ...
           <span class="label label label-info" ng-show="true">{{trans('messages.new_rooms.resubmit')}}</span>
  ...
  <div class="modal fade" id="resubmit_msg_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="resubmit_listing close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title" id="myModalLabel">{{trans('messages.new_rooms.resubmit_listing')}}</h4>
                </div>
                <div class="modal-body">
                
                                <!-- <div class="col-sm-12">
                                <h4>Resubmit reason:</h4>
                                    </div> -->
                                   
                                <div class="col-sm-12">
                                   <span class="resubmitmsg"></span>
                                </div>
                ...
                <div class="modal-footer" style="border-top: 0px solid #e5e5e5;">
                    <button type="button" class="btn btn-default resubmit_listing" data-dismiss="modal">{{trans('messages.new_rooms.close')}}</button>
