...
class PaymentHelper
{
  ...
  public function price_calculation($room_id, $checkin, $checkout, $guest_count, $special_offer_id = '', $change_reservation='', $reservation_id ='')
  {
    ...
    //Availablity check based on availability table 
    //SR_REQUEST_MODIF-03-2019
    
    $month = date('m',strtotime($checkin));
    $year  = date('Y',strtotime($checkin));

    $availability_rules       = $rooms_details->availability()->where(function($query) use($enddate, $date1,$month,$year){
      $query->where('month',$month)->where('year',$year);
    })->orderBy('id', 'desc')->get();

    if($availability_rules->count() > 0) {
      foreach($availability_rules as $k =>$v){
        $minimum_stay =  $v->number_of_month;
      }
    }
    ...
    $result['total_night_price'] = round(($result['total_night_price'] / ($total_nights * 30)) * $total_nights);
    ...
    if($rooms_price->security) {
      ...
      $service_fee  = $this->currency_convert($fee_currency,'',$service_fee_percentage);
    

    $result['service_fee']        = round($service_fee);
    ...
    $result['rooms_price']        = round($result['subtotal']);
    $result['currency']           = $rooms_price->code;

    $result['rental_percentage'] = $rental_percentage;

    
    $result['rental_price'] = round($result['subtotal'] * ($rental_percentage /100));
    ...
    if($reservation)  {
      ...
      $result['rental_price']     = $reservation->rental_prepayment;
      $result['rental_percentage'] = $reservation->rental_percentage;
    }
    //dd($result);
    ...
    if($special_offer && @$special_offer->type == 'special_offer') {
      ...
      $result['rental_price']     = round($special_offer->price * ($rental_percentage /100));
      
    }

    $coupon_amount_total          =  $result['service_fee'] + $result['rental_price'];
    ...
    $result['total']              = ($result['service_fee'] +$result['rental_price'] ) - $result['coupon_amount'];
    $payment_total                = ($result['service_fee'] +$result['rental_price'] ) - $result['coupon_amount'];
    $result['payment_total']      = $payment_total > 0 ? $payment_total : 0;
    $result['payout']             = 0;

    $result['overall_total']      = $result['total'] + $result['subtotal'];

    $result['per_month_price']    = $result['per_night'] + round($result['per_night'] * ($rental_percentage /100));

    return json_encode($result);
  }

  public function price_calculation_backup($room_id, $checkin, $checkout, $guest_count, $special_offer_id = '', $change_reservation='', $reservation_id ='')
  {
    $from                               = new DateTime($checkin);
    $to                                 = new DateTime($checkout);
    $date1                              = date('Y-m-d', strtotime($checkin));
    ...
    foreach($length_of_stay_rules as $rule) {
      if($total_nights >= @$rule['period'] && @$rule['discount'] > 0) {
        $result['length_of_stay_type'] = @$rule['period'] == 7 ? 'weekly' : ( @$rule['period'] == 30 ? 'monthly' : 'custom');
    ...
    public function date_convert_dmy($date)
    {
