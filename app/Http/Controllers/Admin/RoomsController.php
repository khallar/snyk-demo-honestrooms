...
class RoomsController extends Controller
{
    ...
    public function add(Request $request)
    {
        if(!$_POST)
        {
            ...
            //Customization update service , rules , host , bathrooms  in SR_REQUEST_MODIF-12-02-2019
        ...
        {  
         ...
         //check name exists or not            
            ...
            $rooms->cancel_policy_accept = $request->cancel_policy_accept;

            //Add Service , host , rules , bathrooms customization update 12/02/2019

            $rooms->service = $request->service != '' ? implode(',', @$request->service) : '';
 
            $rooms->services_offer     = @implode(',', @$request->services_offer);
 
            $rooms->lives_house_desc = @$request->lives_house_desc;

            $rooms->lives_house      = @implode(',', @$request->lives_house);

            $rooms->speak_language   = @implode(',', @$request->speak_language);

            $rooms->pets             = @implode(',',@$request->pets);

            $rooms->host_summary     = @$request->summary;

            $rooms->allow_smoke      = @$request->allow_smoke;

            $rooms->accept_pets      = @$request->accept_pets;

            $rooms->invite_visitors  =@$request->invite_visitors;

            $rooms->bathrooms      = @$request->bathrooms != '' ? @$request->bathrooms : 0 ;

            $rooms->private_bathrooms      = @$request->private_bathrooms;

            $rooms->status         = 'Listed';

            $rooms->verified       = 'Approve';
            ...
            // $rooms_price->month            = $request->month;
            //remove fileds customization update 14/02/2019
           ...
           // $rooms_price->weekend          = $request->weekend;
            ...
            // $rooms_steps->service     = 1;
            $rooms_steps->host        = 1;
            $rooms_steps->rules       = 1;
            $rooms_steps->bathrooms   = 1;
            ...
            foreach($length_of_stay_rules as $rule) {
                $month_year = explode('-',$rule['period']);
                if(@$rule['id']) {
                    $check = [
                        'id' => $rule['id'],
                        'room_id' => $rooms->id,
                ...
                $price_rule->number_of_month = $rule['discount'];

                $price_rule->save();
            }

            //remove price detail in customization update 11/02/2019
   ...
   if(isset($request->night) || isset($request->week) || isset($request->month))
   {
    ...
    public function update(Request $request, CalendarController $calendar)
    {
        ...
        else if($request->submit == 'pricing')
        {
            ...
            $rooms = Rooms::find($request->room_id);
            $rooms->service = @implode(',', @$request->service);
        ...
        else if($request->submit == 'availability_rules') 
        {
        ...
        else if($request->submit == 'services'){
            $rooms = Rooms::find($request->room_id);
            ...
            // if($rooms->services_offer != ''){
            //     $rooms_steps_status = RoomsStepsStatus::find($request->room_id);
            //     $rooms_steps_status->service = 1;
            //     $rooms_steps_status->save();
            // }
            
            $this->helper->flash_message('success', 'Room Updated Successfully'); // Call flash message function
            return redirect(ADMIN_URL.'/rooms');
        }
        else if($request->submit == 'host'){
            $rooms = Rooms::find($request->room_id);
            ...
            if($rooms->lives_house_desc != '' && $rooms->lives_house != '' && $rooms->speak_language !='' && $rooms->pets != ''){
                $rooms_steps_status = RoomsStepsStatus::find($request->room_id);
                $rooms_steps_status->host = 1;
                $rooms_steps_status->save();
            }
            ...
            if($rooms->allow_smoke != '' && $rooms->accept_pets != '' && $rooms->invite_visitors !=''){
                $rooms_steps_status = RoomsStepsStatus::find($request->room_id);
                $rooms_steps_status->rules = 1;
                $rooms_steps_status->save();
            }
        ...
        else if($request->submit == 'bathrooms'){
            $rooms = Rooms::find($request->room_id);
            ...
            if($rooms->bathrooms != '' && $rooms->private_bathrooms != '' ){
                $rooms_steps_status = RoomsStepsStatus::find($request->room_id);
                $rooms_steps_status->bathrooms = 1;
                $rooms_steps_status->save();
            }
 ...
 public function update_video(Request $request)
    ...
    public function resubmit_listing(Request $request){
        ...
        $message->reservation_id = $request->room_id.''.$room_detail->user_id;
        ...
        // $this->helper->flash_message('success', 'Resubmited Successfully'); // Call flash message function
    ...
    public function update_room_status(Request $request) {
        ...
        if ($user_check->status != 'Active') {
            $this->helper->flash_message('error', 'Not able to ' . $request->type . ' for Not Active users');
            return back();
        }

        if ($room->status == 'Unlisted') {
            $this->helper->flash_message('error', 'Not able to ' . $request->type . ' for unlisted listing');
            return back();
        }
