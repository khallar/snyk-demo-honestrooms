...
class SearchController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $rooms = Rooms::join(
                    ...
                    })->where('rooms_address.latitude')->where('rooms.status','Listed')->where('rooms.cancel_policy_accept','Yes')->get(); 
       ...
       // $data['month']              = $request->input('month')=='' ? 1 : $request->input('month');
        $data['bedrooms']           = $request->input('bedrooms');
        $data['bathrooms']          = $request->input('bathrooms');
        $data['shared_bathrooms']   = $request->input('shared_bathrooms');
        $data['private_bathrooms']  = $request->input('private_bathrooms');
        ...
        $data['services_offer']     = $request->input('services_offer');
        ...
        $data['services_offer_selected'] = explode(',', $request->input('services_offer'));
    ...
    public function searchResult(Request $request) {
       ...
       // dd($request->all());
        ...
        $checkout      = $request->input('checkout');
        // if ($checkin!='') {
        //     $month = $request->input('month') *30;
        //     $checkout = date('d/m/Y', strtotime("+".$month." months", (\DateTime::createFromFormat('d/m/Y',$checkin)->getTimestamp())));
        // }else{
        //     $checkout='';
        // }
        $guest         = $request->input('guest');
        ...
        $services_offer     = $request->input('services_offer');
        $min_price     = $request->input('min_price');
        $max_price     = $request->input('max_price');
        $instant_book  = @$request->input('instant_book');

        $private_bathrooms = $request->input('private_bathrooms');

        $shared_bathrooms  = $request->input('shared_bathrooms');
        ...
        if(!is_array($services_offer))
        {
            if($services_offer != '')
             $services_offer = explode(',', $services_offer);
            else
             $services_offer = [];
        }
        ...
        $rooms_where['rooms.cancel_policy_accept']       = 'Yes';
        ...
        if($rooms_where)
        {
            foreach($rooms_where as $row=>$value)
            {
                if($row == 'rooms.accommodates' ||  $row == 'rooms.bedrooms' || $row == 'rooms.beds')
                    $operator = '>=';
                else
                    $operator = '=';
        ...
        if(count($services_offer))
        {
            foreach($services_offer as $services_offer_value)
                $rooms = $rooms->whereRaw('find_in_set('.$services_offer_value.', services_offer)');
        }
        if($private_bathrooms != ''){
            $rooms = $rooms->where('private_bathrooms',$private_bathrooms);
        }
        if($shared_bathrooms != ''){
            $rooms = $rooms->where('bathrooms',$shared_bathrooms);
        }

        if($dates_available) 
        {
            $rooms = $rooms->where('available_date','<=', $checkin)->where('available_date','!=', '0000-00-00');
        }else{
            $rooms = $rooms->where('available_date','!=', '0000-00-00');
        }
        ...
        //                 ]);
         ...
         // $rooms          = $rooms->join('rooms_price', 'rooms_price.room_id', '=', 'rooms.id');
            ...
            $rooms->whereHas('rooms_price', function($query)use( $currency_rate, $min_price, $max_price, $max_price_check){
                ...
                if($max_price_check >= MAXIMUM_AMOUNT)
                {
                    $query->whereRaw('ROUND(((month / currency.rate) * '.$currency_rate.')) >= '.$min_price);
                }
                else
                {
                    $query->whereRaw('ROUND(((month / currency.rate) * '.$currency_rate.')) >= '.$min_price.' and ROUND(((month / currency.rate) * '.$currency_rate.')) <= '.$max_price);
                }
            });
