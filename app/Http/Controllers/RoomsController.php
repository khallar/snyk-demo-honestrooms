...
use App\Models\RoomsAvailability;
...
class RoomsController extends Controller
{
    ...
    {        
        ...
        $data['unlisted_result'] = Rooms::user()->where(function ($query) {
            $query->where('status', 'Unlisted')->orWhere('status', 'Pending')->orWhere('status', 'Resubmit')->orWhereNull('status');
        })->get(); 
    ...
    public function manage_listing(Request $request, CalendarController $calendar)
    {
        ...
        //Service offer , Lives in house details , Rules of the house update changes
        //SR_REQUEST_MODIF-08-02-2019

        $data['services_offer'] = explode(',',@$data['result']->services_offer);

        //Who lives in the house 

        $data['lives_house']    = Lives::active_all();

        $data['prev_lives_house'] = explode(',',$data['result']->lives_house);

        $data['speak_language'] = Language::where('status','Active')->get();

        $data['prev_speak_language'] = explode(',',$data['result']->speak_language);

        $data['pets'] = Pets::active_all();

        $data['prev_pets'] = explode(',',$data['result']->pets);

        $data['rooms_pets'] = $data['result']->pets;

        $data['service'] = Service::active_all();

        $data['prev_service'] = explode(',',$data['result']->service);
        ...
        $data['availability_month'] = Rooms::getMonthAvailability();

        $data["shared_spaces"] = json_decode($data["result"]->shared_spaces_selected, true);

        if ($data["result"]->shared_spaces_selected == null || json_last_error() != JSON_ERROR_NONE) {
            $data["shared_spaces"] = array(
                "kitchen" => false,
                "living" => false,
                "dining" => false,
                "backyard" => false
    ...
    public function update_rooms(Request $request, EmailController $email_controller)
    {
        ...
        // This function for update steps count in rooms_steps_count table
        
        //Resubmit List
        $rooms = Rooms::find($request->id);

        if($rooms->status == 'Pending' && $rooms->verified == 'Pending' && $rooms->steps_count == '0') {
             if($rooms->send_email == 'No'){
             $email_controller->approve_email_admin($request->id);
             $email_controller->approve_email_host($request->id);   
            }
            $updaterooms= Rooms::find($request->id);
            $updaterooms->send_email = 'Yes';
            $updaterooms->save();
            
        }

        if(($rooms->status == 'Listed' || $rooms->status == 'Resubmit') && ($rooms->verified == 'Approve' || $rooms->verified == 'Resubmit')){
        ...
        $rooms_steps  = RoomsStepsStatus::find($request->id);

        return json_encode(['success'=>'true', 'steps_count' => $rooms->steps_count, 'video' => $rooms->video,'host'=>$rooms_steps->host , 'rules_house' => $rooms_steps->rules ,'bathrooms' =>$rooms_steps->bathrooms,'status' => $rooms1->status]);
    }
    ...
    public function update_status($id)
    {
        ...
        if($result_rooms->property_type != '' && $result_rooms->room_type != '' && $result_rooms->accommodates != '0')
            $rooms_status->basics = 1;
        else
            $rooms_status->basics = 0;


        // //service related updated , update customization 09/02/2019
        // if($result_rooms->services_offer != '')
        //     $rooms_status->service =1;
        // else
        //     $rooms_status->service =0;


        //Host related updated , update customization 09/02/2019

         if($result_rooms->lives_house != '' && $result_rooms->lives_house_desc != '' && $result_rooms->speak_language != '' && $result_rooms->pets != '')
            $rooms_status->host =1;
        else
            $rooms_status->host =0;


        //Rules related updated , update customization 09/02/2019

         if($result_rooms->allow_smoke != '' && $result_rooms->accept_pets != '' && $result_rooms->invite_visitors)
            $rooms_status->rules =1;
        else
            $rooms_status->rules =0;

         //Bathrooms related updated , update customization 12/02/2019

         if($result_rooms->private_bathrooms != ''){
            $rooms_status->bathrooms = 0;
            if($result_rooms->bathrooms != '' && $result_rooms->private_bathrooms == 'No'){
                $rooms_status->bathrooms =1;
            }
            else if($result_rooms->private_bathrooms == 'Yes'){
                $rooms_status->bathrooms =1;
            }
    ...
    {        
     ...
     /**
     * Ajax Update List Your Space Service offer
     * SR_REQUEST_MODIF-08-02-2019
     * @param array $request    Input values
     * @return json success
     */
    public function update_services_offer(Request $request,EmailController $email_controller)
    {        
        $rooms = Rooms::find($request->id);

        $rooms->services_offer = rtrim($request->data,',');
        
        $rooms->save();

        //rooms steps Status update

         $this->update_status($request->id);

       // $email_controller->room_details_updated($request->id, 'Services Offer');
        ...
        return json_encode(['success'=>'true','steps_count' => $rooms->steps_count ,'services_offer' => $rooms->services_offer,'status' => $rooms1->status]);
    }

    /**
     * Ajax Update List Your Space  Lives House
     * SR_REQUEST_MODIF-11-02-2019
     * @param array $request    Input values
     * @return json success
     */
    public function update_lives_house(Request $request,EmailController $email_controller)
    {        
        $rooms = Rooms::find($request->id);

        $rooms->lives_house = rtrim($request->data,',');
        ...
        return json_encode(['success'=>'true', 'steps_count' => $rooms->steps_count,'host'=>$rooms_steps->host,'status' => $rooms1->status]);

        
    }

    /**
     * Ajax Update List Your Space  Speak Language House
     * SR_REQUEST_MODIF-11-02-2019
     * @param array $request    Input values
     * @return json success
     */
    public function update_speak_language(Request $request,EmailController $email_controller)
    {        
        $rooms = Rooms::find($request->id);

        $rooms->speak_language = rtrim($request->data,',');
     ...
     /**
     * Ajax Update List Your Space  Pets
     * SR_REQUEST_MODIF-11-02-2019
     * @param array $request    Input values
     * @return json success
     */
    public function update_pets(Request $request,EmailController $email_controller)
    {      
        $rooms = Rooms::find($request->id);

        $rooms->pets = rtrim($request->data,',');
    ...
    /**
     * Ajax Update List Your Space  Service
     * SR_REQUEST_MODIF-12-02-2019
     * @param array $request    Input values
     * @return json success
     */
    public function update_service(Request $request,EmailController $email_controller)
    {      
        $rooms = Rooms::find($request->id);

        $rooms->service = rtrim($request->data,',');
    ...
    {  //dd($request->all());      
       if($request->type == 'minimum_stay') {
        $month_year = explode('-',@$request->data['year_month']);
        //dd(@$request->data['year_month']);
          $rules    = [
            'year_month'    => 'required|unique:rooms_availability,year,'.@$request->data['id'].',id,month,'.$month_year[1].',room_id,'.$request->id,
            'number_of_month' => 'required|integer|between:1,99',
        ];
       }else{
       ...
       if($request->type == 'minimum_stay') {
         $messages = [
            'year_month.integer'  => trans('validation.numeric', ['attribute' => trans('messages.lys.period')]),
            'number_of_month.integer'  => trans('validation.numeric', ['attribute' => trans('messages.new_rooms.minimum_month')]),
             
        ];
        $attributes = [
            'year_month'    => trans('messages.lys.period'),
            'number_of_month' => trans('messages.new_rooms.minimum_month'),
        ...
        $price_rule = RoomsAvailability::firstOrNew($check);
        $price_rule->room_id = $request->id;
        $price_rule->month =  $month_year[1];
        $price_rule->year = $month_year[0];
        $price_rule->number_of_month = $rule['number_of_month'];
    ...
    public function delete_price_rule(Request $request) {
        ...
        $id = $request->rule_id;
        if($request->type == 'minimum_stay'){
           RoomsAvailability::where('id', $id)->delete();
        }else{
    ...
    {   
        ...
        if($data['user_details']->status != 'Active' && $data['result']->user_id != @Auth::user()->id)
           abort('404');
            ...
            abort('404');
        else if($data['result']->cancel_policy_accept != 'Yes' && $data['result']->user_id != @Auth::user()->id)
            abort('404');
        ...
        $data['amenities']        = collect(Amenities::selected($request->id));

        $idRoomAmenities = AmenitiesType::where('name', 'Room Amenities')->first()->id;
        $data['room_amenities']   = $data['amenities']->where('type_id', $idRoomAmenities);
        $data['other_amenities']  = $data['amenities']->where('type_id', '!=', $idRoomAmenities);
        ...
        //Service offer , host , rules and bathrooms detail view customization updated in 13/02/2019

        $data['services_offer']   = explode(',',$data['result']->services_offer);

        $data['lives_house']      = explode(',',$data['result']->lives_house);

        $data['lives']      = Lives::active_all();

        $data['language1']   = Language::where('status','Active')->get();

        $data['speak_language']   = explode(',',$data['result']->speak_language);

        $data['pets']  = Pets::active_all();

        $data['pets_val'] = explode(',',$data['result']->pets);

        $data['service'] = Service::active_all();

        $data['service_val'] = explode(',',$data['result']->service);

        $data['service_count'] = count($data['service_val']);
        ...
        // if($request->checkin != '' && $request->month == '')
        // {
        //     $data['checkin']         = date('m/d/Y', $this->helper->custom_strtotime($request->checkin));
        //     $data['checkout']        = date('m/d/Y', strtotime('+30 day',$this->helper->custom_strtotime($request->checkin)));
        //     $data['formatted_checkin'] = date('d-m-Y',$this->helper->custom_strtotime($request->checkin));
        //     $data['formatted_checkout'] = date('d-m-Y',strtotime('+30 day',$this->helper->custom_strtotime($request->checkin)));
        //     if(@$data['result']['accommodates'] >= @$request->guests){
        //         $data['guests']          = $request->guests;}else{
        //         $data['guests']          = '1';
        //     }
        //      $data['numberofmonth']          = '1';
        // }
        // else if($request->checkin != '' && $request->month != '')
        // {
        //     $month = $request->month *30;
        //     $data['checkin']         = date('m/d/Y', $this->helper->custom_strtotime($request->checkin));
        //     $data['checkout']        = date('m/d/Y', strtotime('+'.$month.' day',$this->helper->custom_strtotime($request->checkin)));
        //     $data['formatted_checkin'] = date('d-m-Y',$this->helper->custom_strtotime($request->checkin));
        //     $data['formatted_checkout'] = date('d-m-Y',strtotime('+'.$month.' day',$this->helper->custom_strtotime($request->checkin)));
        //     if(@$data['result']['accommodates'] >= @$request->guests){
        //         $data['guests']          = $request->guests;}else{
        //         $data['guests']          = '1';
        //     }
        //      $data['numberofmonth']          = $request->month;
        // }
        // else
        // {
        //     $data['checkin'] = '';
        ...
        $data['similar']          = Rooms::join('rooms_address', function($join) {
                                    ...
                                    ->where('rooms.cancel_policy_accept','Yes')
 ...
 public function remove_video(Request $request){
    ...
    public function update_price(Request $request,EmailController $email_controller)
    {
        ...
        $currency_symbol = Currency::whereCode($data->currency_code)->first()->original_symbol;
         ...
         //Night to Month customizatio => SR_REQUEST_MODIF-12-02-2019
        if(isset($data->month))
        {
            $old_currency_format = RoomsPrice::find($request->id);
            $month_price = $data->month;
            if(is_numeric($month_price) && $month_price < $minimum_amount)
            {
        ...
        return json_encode(['success'=>'true', 'currency_symbol' => $price->currency->original_symbol, 'steps_count' => $price->steps_count,'month_price'=>$month_price,'status' => $rooms1->status]);
    }
    ...
    {   
        ...
        $result['price'] = RoomsPrice::where('room_id', $id)->get()->pluck('month');
        //get weekend price
        ...
        $result['available_date'] = $rooms->available_date == '0000-00-00' ? '' : $rooms->available_date_mdy ;
...
// Ajax Check Date availability 
    ...
    public function become_a_host(){
        return view('host.become_a_host');
    }
  ...
  /**
   * Rooms Available Date update
   * @param int $room_id
   * @param date $available_date
   */
  public function rooms_available_date(Request $request){
 
    $rooms = Rooms::find($request->room_id);
    $rooms->available_date =  date('Y-m-d', $this->helper->custom_strtotime($request->available_date));
    $rooms->save();
    return redirect('manage-listing/'.$request->room_id.'/completed');
