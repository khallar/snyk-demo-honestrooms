...
class HomeController extends Controller {
	...
	public function index() {
		...
		$data['popular_rooms'] = Rooms::where('status', 'Listed')->where('cancel_policy_accept','Yes')->get();
		...
		$data['recommented'] = Rooms::whereHas('users', function ($query) {$query->where('status', 'Active');})->orderBy('id', 'desc')->where('recommended', 'Yes')->where('status', 'Listed')->where('cancel_policy_accept','Yes')->groupBy('id')->get();
	...
	public function ajax_home() {
		...
		}, 'currency'])->selectRaw('*, max(id) as reservation_id')->where('list_type', 'Rooms')->whereHas('rooms', function ($query) {$query->where('status', 'Listed')->where('cancel_policy_accept','Yes')->where('available_date','!=', '0000-00-00');})->whereHas('host_users', function ($query) {$query->where('status', 'Active');})->orderBy('reservation_id', 'desc')->where('status', 'Accepted')->groupBy('room_id')->limit(10)->get();

		$data['recommented'] = Rooms::with(['rooms_price' => function ($query) {
			$query->with('currency');

		}])->whereHas('users', function ($query) {$query->where('status', 'Active');})->orderBy('id', 'desc')->where('recommended', 'Yes')->where('available_date','!=', '0000-00-00')->where('status', 'Listed')->groupBy('id')->get();

		$data['view_count'] = Rooms::with(['rooms_price' => function ($query) {
			$query->with('currency');

		}])->whereHas('users', function ($query) {$query->where('status', 'Active');})->orderBy('views_count', 'desc')->where('status', 'Listed')->where('available_date','!=', '0000-00-00')->groupBy('id')->get();
