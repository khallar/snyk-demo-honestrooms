...
class PaymentController extends Controller 
{
    ...
    public function index(Request $request)
    {
        ...
        $data['nights']           = ($to->diff($from)->format("%a") / 30);
        $data['total_nights']           = ($to->diff($from)->format("%a"));
        ...
        if($data['result']->user_id == $data['user_id']){
           ...
           if($data['result']->cancel_policy_accept != 'Yes'){
            return redirect('rooms/'.$data['room_id']);
        }
    ...
    {        
        if($request->session_key && $request->session_key !='')
        {
            ...
            $reservation->rental_prepayment  = $data['price_list']->rental_price;
            $reservation->rental_percentage  = $data['price_list']->rental_percentage;
            ...
            // $reservation->length_of_stay_type           = @$data['price_list']->length_of_stay_type ? @$data['price_list']->length_of_stay_type : 0;
    ...
    public function store($data)
    {
        ...
        {   
            ...
            //Customization host amount directly paid if no host payout 
