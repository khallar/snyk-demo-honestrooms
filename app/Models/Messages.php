...
class Messages extends Model
{
    ...
    protected $appends = ['created_time','pending_count','archived_count','reservation_count','unread_count','stared_count','all_count','host_check','guest_check','inbox_thread_count','admin_profile_src','message_split'];
    ...
    //Get Admin Profile Src
    public function getAdminProfileSrcAttribute(){
        $src = url('admin_assets/dist/img/avatar04.png');
        return $src;
    }

    // Get Unread Message Count
    public  function getMessageSplitAttribute()
    {
        if(strlen($this->attributes['message']) <= 40){
            return substr($this->attributes['message'],0,40);
        }else{
            return substr($this->attributes['message'],0,40).' ...';
        }
