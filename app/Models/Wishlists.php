...
class Wishlists extends Model
{
    ...
    {
        return @DB::table('saved_wishlists')->where('saved_wishlists.list_type','Rooms')->where('saved_wishlists.wishlist_id', $this->attributes['id'])->join('rooms', 'rooms.id' ,'=', 'saved_wishlists.room_id')->where('rooms.status','Listed')->where('rooms.cancel_policy_accept','Yes')->where('saved_wishlists.user_id', $this->attributes['user_id'])->count();
    }

    public function getAllRoomsCountAttribute()
    {
    	return @DB::table('saved_wishlists')->where('list_type','Rooms')->where('wishlist_id', $this->attributes['id'])->join('rooms', 'rooms.id' ,'=', 'saved_wishlists.room_id')->where('rooms.status','Listed')->where('rooms.cancel_policy_accept','Yes')->count();
    }
