...
class Reservation extends Model {
	...
	public function getHostPayoutAttribute() {
		...
		if ($check->count()) {
			...
			return $this->currency_calc('subtotal')+$this->currency_calc('total') - $this->currency_calc('service') - $this->currency_calc('host_fee') + $this->currency_calc('coupon_amount')-$this->currency_calc('rental_prepayment') ;
		}
	...
	public function getCheckTotalAttribute() {
		...
		*/
		...
		if ($host_id == $user_id) {
			return $this->currency_calc('total') + $this->currency_calc('coupon_amount') - $this->currency_calc('service') - $this->currency_calc('rental_prepayment') - $this->currency_calc('host_fee')-@$this->hostPayouts->total_penalty_amount;
		} else {
	...
	public function getGrandTotalAttribute() {
		...
		if ($host_id == @Auth::user()->id) {
			return $this->currency_calc('subtotal') + $this->currency_calc('coupon_amount') - $this->currency_calc('service') -$this->currency_calc('rental_prepayment');
		} else {
	...
	public function getDurationTextAttribute() {
		...
		$duration_text = $duration . ' ' . trans_choice('messages.service_host_rule.day', $duration);
		if (@$this->attributes['list_type'] == 'Experiences') {
	...
	public function getDurationTypeTextAttribute() {
		$duration = $this->duration;
		$duration_text = trans_choice('messages.service_host_rule.day', $duration);
		if (@$this->attributes['list_type'] == 'Experiences') {
    ...
    public function getRentalPrepaymentAttribute() {
		return $this->currency_calc('rental_prepayment');
	}
	...
	*/
	...
	//Get overall total Attribute
	public function getOverallTotalAttribute() {
		
			return $this->currency_calc('subtotal') + $this->currency_calc('total');
		
	}

	public function getNightsCountAttribute(){
		 $from                     = new DateTime($this->attributes['checkin']);
         $to                       = new DateTime($this->attributes['checkout']);
